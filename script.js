...
function updateJobTable() {
    ...
    jobs.forEach((job, index) => {
        ...
        row.innerHTML = `
            <td>J${job.id}</td>
            <td><input type="number" class="form-control" step="0.1" value="<span class="math-inline">\{job\.arrivalTime\}" min\="0" onchange\="updateJobProperty\(</span>{index}, 'arrivalTime', this.value)"></td>
            <td><input type="number" class="form-control" step="0.1" value="<span class="math-inline">\{job\.burstTime\}" min\="0\.1" onchange\="updateJobProperty\(</span>{index}, 'burstTime', this.value)"></td>
            <td><span class="math-inline">\{job\.startTime \=\=\= \-1 ? '\-' \: job\.startTime\.toFixed\(1\)\}</td\>
<td\></span>{job.endTime.toFixed(1)}</td>
        ...
        for (let i = 0; i < cpuCount; i++) {
            if (runningJobs[i] !== null) {
                ...
                if (job.remainingTime === 0) {
                    ...
                    completedJobs++;
                    runningJobs[i] = null;
                } else if (runningJob.allocatedTime >= timeQuantum) {
                    // Job's time quantum is up, will be moved back to queue at the next time quantum boundary
                    runningJobs[i] = null;
                }
            } else {
                jobHistory.push({
...
function calculateAverageTurnaroundTime() {
    ...
    document.getElementById("averageTurnaroundTime").innerHTML = `
        Average Turnaround Time: (${calculation}) / <span class="math-inline">\{jobs\.length\} \= <b\></span>{result}</b>
    `;
}
